...
public class Q15 {
    class Solution {
        public int solution(int[] nums) {
            ...
            //[3,1,2,3] -> size() -> 3, 배열 길이/2 ->2
            if(set.size() > max) { // size 3 > max 2
                return max; //return 2
            } else { //[3,3,3,2,2,2] -> size() -> 2, 배열길이/2 ->3
                return set.size(); // size 2 < max 3 --> size return
            }
        }
    }
}
...
N마리 폰켓몬의 종류 번호가 담긴 배열 nums가 매개변수로 주어질 때,
 N/2마리의 폰켓몬을 선택하는 방법 중, 가장 많은 종류의 폰켓몬을
 선택하는 방법을 찾아, 그때의 폰켓몬 종류 번호의 개수를
...
입출력 예 #3
6마리의 폰켓몬이 있으므로, 3마리의 폰켓몬을 골라야 합니다.
가장 많은 종류의 폰켓몬을 고르기 위해서는 3번 폰켓몬 한 마리와
 2번 폰켓몬 두 마리를 고르거나, 혹은 3번 폰켓몬 두 마리와
  2번 폰켓몬 한 마리를 고르면 됩니다.
