...
public class StaticEx {
    ...
    /* Static 멤버 선언
    필드나 메소드를 생성 시 인스턴스로 생성할 것인지 정적으로 생성할 것인지에 대한
    판단 기준은 공용으로 상용하느냐 아니냐로 내리면 됩니다.
    그냥 생성한다면 자동으로 인스턴스로 생성되며 정적으로 생성하려면
    ...
    * */
    ...
    public static void main(String[] args) {
        ...
        /* 인스턴스 변수는 인스턴스가 생성될 때마다 생성되므로
           인스턴스가 각기 다른 값을 가지지만 정적 변수는 모든
           인스턴스가 하나의 저장공간을 공유하기에 항상 같은 값을
           가지기에 나타난 현상
        * */
...
/* Static
  ...
  다른 말로는 정적 필드와 정적메소드라고도 하여 이 둘을 합쳐 클래스 멤버라고도 합니다.
  정적 필드와 정적 메소드는 객체에 소속된 멤버가 아니라 클래스에 고정된 멤버입니다.

  Static 멤버 생성

  Static 키워드를 통해 생성된 정적 멤버들은 Heap 영역이 아닌 Static 영역에
  할당됩니다. Static 영역에 할당된 메모리는 모든 객ㅊ에가 공유하여 하나의
  멤버를 어디서든지 참조할 수 있는 장점을 가지지만 Garbage Collector의 관여 영역
  밖에 존재하기에 Static영역에 있는 멤버들은 프로그램의 종료시까지
  메모리에 할당된 재촐 존재하게 됩니다. 그렇기에 Static을 너무 남발하게 되면
